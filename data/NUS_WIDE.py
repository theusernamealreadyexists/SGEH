...
class NUS_map(torch.utils.data.Dataset):
    def __init__(self, args):
        try:
            sim_map = scio.loadmat(osp.join(args.SIM_MAP + 'nus_train_uni.mat'))
            sim = np.asarray(find(sim_map['U'])).T

        except Exception:
            sim_map = h5py.File(osp.join(args.SIM_MAP + 'nus_train_uni.mat'), 'r')
            sim = np.asarray(find(sim_map['U'])).T
            sim_map.close()
